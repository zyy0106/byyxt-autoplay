...
2. 进入博雅云学堂https://byyxt.pupedu.cn &rarr; 进入你所选的课程&rarr;手动使用「统一身份认证」进入课程页&rarr;点击「进入学习任务」&rarr;复制浏览器顶端的url（需要包含“statudentsHomeDetails”字符串的那一个链接）并保管好&rarr;手动退出登录并退出博雅云学堂网页【**必须每一步都严格按照说明进行**】;
